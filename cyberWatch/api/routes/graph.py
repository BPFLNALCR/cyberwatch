...
async def neighbors(asn: int, driver: AsyncDriver = Depends(neo4j_dep)):
    ...
    async with driver.session() as session:
        records = await session.execute_read(lambda tx: list(tx.run(query, asn=asn)))
    data = [
        {
...
async def shortest_path(
    ...
    """
    async with driver.session() as session:
        record = await session.execute_read(lambda tx: tx.run(query, src=src_asn, dst=dst_asn).single())
    if record is None:
