...
# Internal domains used by CyberWatch for enrichment that should always be ignored
CYBERWATCH_INTERNAL_SUFFIXES = (
    ".origin.asn.cymru.com",      # Team Cymru ASN lookups
    ".peer.asn.cymru.com",        # Team Cymru peer lookups
    ".asn.cymru.com",             # Other Cymru lookups
    ".rpki.cloudflare.com",       # RPKI validation
    ".whois.arin.net",            # ARIN whois
    ".whois.ripe.net",            # RIPE whois
)
...
def _ignore_query(cfg: DNSCollectorConfig, query: DNSQuery) -> bool:
    ...
    if cfg.filters.max_domain_length and len(domain) > cfg.filters.max_domain_length:
        ...
        return True
    # Always ignore reverse DNS lookups
    if domain.endswith((".in-addr.arpa", ".ip6.arpa")):
        return True
    # Always ignore CyberWatch's own enrichment lookups
    if domain.endswith(CYBERWATCH_INTERNAL_SUFFIXES):
        logger.debug(f"Ignoring CyberWatch internal lookup: {domain}")
        return True
    # Check if domain looks like a reverse IP lookup pattern (e.g., 1.2.3.4.origin.asn...)
    # These are generated by CyberWatch for ASN enrichment
    parts = domain.split(".")
    if len(parts) >= 4:
        # Check if first 4 parts are all digits (like 166.96.244.77.origin...)
        if all(part.isdigit() for part in parts[:4]):
            logger.debug(f"Ignoring IP-prefixed domain (likely ASN lookup): {domain}")
            return True
